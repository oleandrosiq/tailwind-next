...
const fileItem = tv({
	slots: {
		...
		icon: 'border-biolet-100 rounded-full border-4 bg-violet-200 p-2 text-violet-400',
		deleteButton: ''
	},
...
type TFileItemProps = VariantProps<typeof fileItem> & {
	name: string;
	size: number;
};

export const FileItem = ({ name, size, state }: TFileItemProps) => {
